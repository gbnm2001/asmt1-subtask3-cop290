...
int main(int argc,char* argv[]){
    ...
    vector<Point2f> pts_dstb;
    pts_dstb.push_back(Point2f(0, 0));//left up
    pts_dstb.push_back(Point2f(0, 800));//left bottom
    pts_dstb.push_back(Point2f(400, 800));//right bottom
    pts_dstb.push_back(Point2f(400, 0));//right up

	Mat h = findHomography(pts_src, pts_dst);
	Mat hb = findHomography(pts_src,pts_dstb);
    //warped image
    Mat warp_back;
    Mat warp_backb;
    // Warp source image to destination based on homography
    warpPerspective(back_image, warp_back, h, back_image.size());
    warpPerspective(back_image, warp_backb, hb, back_image.size());
	...
	while (true){
		...
		if(fm_count%5 == 1){
            ...
            warpPerspective(frameb, warp_frameb, hb, back_image.size());
    		// Display image
            ...
            //cout << fm_count -2 << ',' << qdb/(320000.0) << ',' << ddb/(320000.0) << endl;
  			err =pow((qd/(x*y) - qdb/(320000.0)),2);
			if (err != 0){
